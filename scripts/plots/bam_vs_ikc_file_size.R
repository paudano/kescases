...
plot.base <- ggplot(df.size, aes(bam_size, ikc_size, color=removed)) +
    ...
    theme(legend.justification=c(1, 1), legend.position=c(1, 0.95), legend.background=element_rect(color='black'), legend.key.size=unit(1, 'cm'), legend.text=element_text(size=18))
